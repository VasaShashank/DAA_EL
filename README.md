...
- **Why This?**: Dijkstra is chosen over Bellman-Ford or Floyd-Warshall because all road distance metrics are strictly non-negative ($w(u,v) \ge 0$). Bellman-Ford is unnecessary as there are no negative weight cycles. Floyd-Warshall ($O(V^3)$) is too slow for real-time simulation on maps where we only need single-source paths. Using a Min-Heap reduces the minimum node extraction cost from $O(V)$ to $O(\log V)$, making routing lightning fast.

### 2. Traffic-Aware Dynamic Dijkstra
- **Complexity**: $O((V + E) \log V)$
- **Heuristic Cost Formula**: $\text{Cost} = \text{Distance} + \text{Traffic Weight} \times 2 + \text{Condition Penalty} \times 3 + \text{Construction Delay} \times 4$
...
- **Environmental Metrics**: 
  - $\text{Fuel} = \text{Distance} \times 0.32 \text{ L/km}$
  - $\text{CO}_2 = \text{Fuel} \times 2.68 \text{ kg CO}_2/\text{L}$
...
1. **Shortest Distance Mode**:
   ...
   - **Formulas**:
     - $\text{Distance} = D_{\text{shortest}}$ (Baseline)
     - $\text{Fuel} = D_{\text{shortest}} \times 0.32\text{ L/km}$
     - $\text{CO}_2 = \text{Fuel} \times 2.68\text{ kg CO}_2/\text{L}$
     - $\text{Time} = D_{\text{shortest}} \times 0.5\text{ min/km}$
...
2. **Traffic-Aware Mode**:
   ...
   - **Formulas**:
     - $\text{Distance} = \text{Sum of physical edge lengths along the traffic-weighted path}$ (often slightly longer geometrically, but faster).
     - $\text{Fuel} = D_{\text{traffic}} \times 0.6 \times 0.32\text{ L/km}$ (Fuel consumption rate is reduced by $40\%$ due to avoiding stop-and-go idling in congested areas).
     - $\text{CO}_2 = \text{Fuel} \times 2.68\text{ kg CO}_2/\text{L}$
     - $\text{Time} = (\text{Accumulated Traffic Edge Weights}) \times 0.3\text{ min/unit}$
...
3. **Lowest Fuel Mode**:
   ...
   - **Formulas**:
     - $\text{Distance} = D_{\text{shortest}} \times 1.1$ (Allows a detour of up to $10\%$ to bypass congestion).
     - $\text{Fuel} = D_{\text{shortest}} \times 0.9 \times 0.32\text{ L/km}$ (Reflects $10\%$ fuel savings by avoiding deceleration/acceleration cycles).
     - $\text{CO}_2 = \text{Fuel} \times 2.68\text{ kg CO}_2/\text{L}$
     - $\text{Time} = \text{Distance} \times 0.5\text{ min/km} \times 1.1$
...
4. **Lowest Carbon Mode**:
   ...
   - **Formulas**:
     - $\text{Distance} = D_{\text{shortest}} \times 1.15$ (Allows a detour of up to $15\%$).
     - $\text{Fuel} = D_{\text{shortest}} \times 0.85 \times 0.32\text{ L/km}$ ($15\%$ fuel savings).
     - $\text{CO}_2 = \text{Fuel} \times 2.68\text{ kg CO}_2/\text{L} \times 0.9$ (Includes an extra $10\%$ reduction in emissions by assuming steady-state cruising velocities).
     - $\text{Time} = \text{Distance} \times 0.52\text{ min/km}$ (reflecting slightly slower, eco-friendly driving speeds).

### 🏆 How the BEST Route is Selected
When the user clicks on an optimization goal in the header menu:
- **Distance**: The reducer function selects the route where $\text{Distance}$ is minimized.
- **Fuel**: Selects the route where $\text{Fuel}$ consumption is minimized.
- **Carbon**: Selects the route where $\text{CO}_2$ emissions are minimized.
- **Time**: Selects the route where travel $\text{Time}$ is minimized.
...
1. **The $N \le 8$ TSP Threshold Rationale**:
   - **Computational Complexity**: The Traveling Salesperson Problem has a factorial time complexity of $O(n!)$. 
   - **JavaScript Event Loop Constraint**: For $n = 8$ scheduled bins (plus $1$ Depot node = $9$ total graph vertices), the search space contains $9! = 362,880$ permutations. Utilizing Branch & Bound bounds pruning, the solver evaluates less than $1,000$ states on average, completing in **$1 - 5\text{ ms}$**. 
   - For $n > 8$, the worst-case factorial expansion exceeds $3.6 \times 10^6$ states. This blockages the single-threaded JavaScript execution stack, dropping frame rates below $60\text{ fps}$ and freezing the UI. To prevent this, a hard threshold is set to fall back to the $O(n^2)$ Nearest Neighbor heuristic.
...
2. **Graph Size & Depot Capacity Limits**:
   ...
   - **Node Density Boundary**: The generator is bounded between **$5$ and $25$ bins** ($6$ to $26$ total nodes). A fully connected graph of $V$ vertices contains $E = \frac{V(V-1)}{2}$ edges. Generating beyond $25$ bins yields $>325$ active links, which overlaps rendering coordinates and clutters the visual map display.
...
*   **The Heavy Node Defense**: If a bin has a very high priority score (e.g. near a hospital or has many complaints), its priority value $p_i$ is exceptionally high. In dynamic programming table state transitions, a high-value item is highly favored by the recurrence formula:
    $$\text{dp}[i][w] = \max(\text{dp}[i-1][w], \text{dp}[i-1][w-w_i] + p_i)$$
...
*   **Starvation Prevention & Priority Progression**: Bins are prioritized using a multi-variable dynamic heuristic formula:
    $$\text{Priority} = (\text{Fill Level} \times 0.4) + (\text{Complaints} \times 5) + (\text{Hospital? (20)}) + (\text{School? (15)}) + (\text{Smell Score} \times 3) + (\text{Density} \times 0.1)$$
    - **How this prevents starvation**: 
      ...
      - Within a few simulation cycles, the priority score of Bin 4 grows rapidly:
        $$\text{Initial Priority (80\% fill)} \approx 32.0 \rightarrow \text{5 Ticks Later (100\% fill, 5 complaints, 5 smell)} \approx 40 + 25 + 15 = 80.0$$
...
*   **Modified Edge Weights**: Instead of running Dijkstra using pure geometric distance, the edge weights $w(u,v)$ are modified:
    *   **Time**: $\text{Edge Cost} = \frac{\text{Distance}}{\text{Speed}} + \text{Traffic Delay}$.
    *   **Fuel**: $\text{Edge Cost} = \text{Distance} \times \text{Consumption Rate} \times \text{Congestion Multiplier}$.
*   **Carbon Footprint**: Carbon emissions are directly proportional to fuel burned ($\text{Fuel} \times 2.68\text{ kg CO}_2/\text{L}$). Minimizing fuel consumption dynamically minimizes the total carbon footprint, preferring steady-speed detours over congested shortcuts.
...
- **Spacious Roads**: Network links are scaled to $3.5\text{px} - 6.5\text{px}$ width with glow filters, dynamic traffic coloring (green/orange/red), and road failure markers.
